...
if not os.path.exists('./test pic'):
    os.makedirs('./test pic')
...
def qr_code_scanner():
    """Thread function to continuously scan QR codes and take screenshots."""
    while True:
        ret, frame = cap.read()
        if not ret:
            break
...
try:
    # Start the QR code scanning and screenshot thread
